...
const char* gray_effect_vert = "#version 430\n"\
"layout(location = 0) in vec3 Vertex; \n"\
"out vec2 texCoord;\n"\
"void main(void) {\n"\
"	texCoord = Vertex.xy;\n"\
...
const char* gray_effect_frag = "#version 430\n"\
"uniform sampler2D texture;\n"\
"in vec2 texCoord;\n"\
"out vec4 out_color;\n"\
"void main(void) {\n"\
"	vec4  src_color = texture2D(texture, texCoord);\n"\
"	float g =  0.21 * src_color.r + 0.72 * src_color.g + 0.07 * src_color.b;"
"	out_color =  vec4(g,g,g,src_color.a);\n"\
"}\n";
...
CShader::CShader(std::string type) {
	static std::map<std::string, std::vector<std::vector<const char*>>> list = {
		...
		},
		{
			"GrayEffect",
			{
				{gray_effect_vert,gray_effect_frag},
				{gray_effect_vert,gray_effect_frag},
				{gray_effect_vert,gray_effect_frag},
			}
		},
